...
from src.evaluations.datasets.generator.synthesizer import (
    BaseSynthesizer,
    CustomSynthesizer,
    DeepEvalSynthesizer,
    SynthesizerConfig,
...
async def example_custom_synthesizer_dual_modes():
    ...
    synthetic_examples = await synthetic_synthesizer.generate_synthetic_dataset(
        workflow=workflow, num_scenarios=2
    )

    print(f"\n📊 SYNTHETIC Mode Results:")
    print(f"   - Generated examples: {len(synthetic_examples)}")
    print(
        f"   - Mode: {synthetic_examples[0].metadata.get('execution_mode') if synthetic_examples else 'N/A'}"
    )
    ...
    print(
        f"   - Mode: {execution_examples[0].metadata.get('execution_mode') if execution_examples else 'N/A'}"
    )
    ...
    print(f"\n🔍 Key Differences:")
    print(
        f"   • SYNTHETIC: Fast generation, simulated responses, no infrastructure load"
    )
...
async def example_deepeval_with_enhancements():
    ...
    if basic_examples:
        ...
        print(
            f"   Basic: workflow={basic_context.get('workflow_enhanced')}, task={basic_context.get('task_enhanced')}"
        )

    if custom_examples:
        enhanced_context = custom_examples[0].metadata.get("generation_context", {})
        print(
            f"   Enhanced: workflow={enhanced_context.get('workflow_enhanced')}, task={enhanced_context.get('task_enhanced')}"
        )

        # Show enhanced tags
        enhanced_tags = [
            tag
            for tag in custom_examples[0].metadata["tags"]
            if tag.startswith("enhanced:")
        ]
        ...
        try:
            if isinstance(synthesizer, CustomSynthesizer):
                ...
                examples = await synthesizer.generate_synthetic_dataset(
                    workflow=workflow, num_scenarios=1
                )
            else:
...
async def example_production_workflow():
    ...
    try:
        ...
        volume_examples = await volume_synthesizer.generate_synthetic_dataset(
            workflow=workflow, num_scenarios=5
        )

        print(f"   ✅ Volume: {len(volume_examples)} examples")

        total_examples = (
            len(baseline_examples) + len(validation_examples) + len(volume_examples)
        )
...
async def main():
    ...
    try:
        ...
        print(
            "🎯 DeepEvalSynthesizer: task_description & scenario parameters demonstrated"
        )
